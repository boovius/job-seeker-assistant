# Experience Database

_Last updated: 2026-04-06_

## Canonical Entries

### Book Bites Inc.
- **Canonical title:** Founder / Technical Consultant
- **Dates:** 2021–Present
- **Location:** Remote
- **Summary:** Provides end-to-end architecture, product, and engineering leadership for startups and creative technology teams, from early discovery through system design, delivery, and iteration.
- **Responsibilities / evidence:**
  - End-to-end architecture and engineering leadership
  - Production mobile development across iOS and Android
  - Architecture, delivery, and long-term maintainability
  - Advisory work for early-stage teams on technical architecture, delivery planning, and data-driven product strategy
- **Client / project subentries:**
  - **Clipper Digital:** Modernization of legacy Android and iOS applications
  - **Campaign Finance Board (CFB) — Contribute app:** Product management across a team of 3 engineers and a set of product-owning clients; managed and drew out roadmap, customer research, data analysis, business initiative driving, prioritization, PRD creation, and pushed the product forward while maintaining and improving core stability. Led an overhaul of emergency operations when the product was flooded with contributions from the Mamdani campaign in New York City, updating internal operations to handle much higher data volumes with greater resiliency and faster response times. Helped complete a long-stalled modernization epic and is now supporting development of new user-facing sharing/growth features and improved internal tooling.
  - **Everplans:** Senior mobile iOS work focused on security concerns, maintenance, feature development, and delivery
- **Keywords / tags:** architecture, mobile, iOS, Android, modernization, product management, roadmap, customer research, data analysis, prioritization, PRDs, delivery planning, technical consulting, startup, product strategy, cross-functional
- **Source(s):** Resume 2026-04-06; user clarification 2026-04-06
- **Ambiguities / follow-up needed:**
  - What concrete outcomes or shipped products can be described for each client?
  - Was this full-time founder work, consulting, or a mix?

### Frequency Machine / Cerca
- **Canonical title:** Head of Product Management / Engineering Lead
- **Dates:** 2021–2024
- **Location:** Remote
- **Company / product relationship:** Frequency Machine is the company; Cerca is the product.
- **Team / reporting scope:** Small cross-functional team with up to 3 engineers at peak, 2 designers across different periods, plus support staff across customer operations, marketing/communications, and business/strategy; served as a central funnel translating across those groups into product development.
- **Summary:** Joined as the first technical hire in a founding-technical-role capacity and helped define, build, launch, and scale Cerca, a media and social connection application built around valorizing local expert knowledge about places.
- **Responsibilities / evidence:**
  - Served as Head of Product Management while also acting as engineering lead in a heavily hands-on capacity
  - Helped stakeholders discover and define what the product should be through customer research and product discovery
  - Led design in discovery sessions
  - Managed initial technical hiring across engineering and design, including performance management, firing, and new hiring
  - Brought the product to the App Store and scaled it to a modest user base
  - Performed substantial individual contributor engineering work building the media application and social information-sharing product
  - Re-architected core data access patterns and system flows
  - Improved app startup time by 400% for users participating in multiple conversations by redesigning chat-state handling around Twilio events and persisting conversation interaction state, preventing expensive unnecessary processing during app launch
  - Designed and implemented API-driven systems, including real-time communication services and event-driven workflows
  - Aligned engineering execution with product and business priorities
- **Keywords / tags:** head of product management, engineering lead, founding technical hire, product discovery, customer research, design discovery, hiring, performance management, app store launch, mobile product, Twilio, performance optimization, APIs, event-driven systems, cross-functional, delivery
- **Source(s):** Resume 2026-04-06; user clarification 2026-04-06
- **Key milestones:**
  - Broke ground on the product: December 2021
  - Released to App Store: May 2022
  - Released Concierge matching: September 2022
  - Enabled first monetary exchanges: August 2023
- **Ambiguities / follow-up needed:**
  - Any specific user, launch, or business outcomes that can be cited beyond milestone dates?

### Def Method
- **Canonical title:** Senior Software Engineer / Engineering Manager
- **Dates:** 2016–2021
- **Location:** New York, NY
- **Title progression:** Started as Software Engineer in 2016, promoted to Senior Software Engineer in 2019, and took on Engineering Manager responsibilities in May 2021; senior engineering and engineering management titles overlapped.
- **Summary:** Product-focused technologist and lead engineer delivering complex mobile and web applications across multiple consulting engagements, balancing velocity with maintainability while growing from contributor to team lead and engineering manager.
- **Responsibilities / evidence:**
  - Managed development teams
  - Led design and delivery of data-intensive APIs and integrations
  - Conducted performance reviews
  - Mentored engineers
  - Established best practices for sustainable delivery
- **Client / project subentries:**
  - **Facebook (Fall 2020–Spring 2021):** Helped overtake the sunsetting of an internal design application and support rapid adoption of new tooling (Universe); later contributed React frontend work for product discovery on a 1:1 video conferencing tool
  - **Pager (Spring 2020):** Engineering team lead for a prototype exploring a new type of social networking / communication app
  - **Jetblack / Walmart Labs (2019):** Engineering team lead for a 5-person squad building a greenfield app to coordinate last-mile delivery for a high-end boutique consumer goods delivery service. Although the project was cut short when the company folded earlier than expected, led the engineering team in task delivery, hands-on engineering, teamwork, project planning, and product discovery/observation; helped align the team with the broader organization and coordinated closely with DevOps, machine learning, and other adjacent teams.
  - **JOOR (Fall 2017–Spring 2018):** Python contributor to an ETL pipeline ingesting and normalizing large volumes of client data; contributed to DevOps modernization including early Kubernetes orchestration and Docker containerization; led and coached QA engineers to modernize their work
  - **Casper / NYC Landlords / FloodHelp (2016):** Contributor across multiple early client engagements, including FloodHelp, a flood insurance warning/information tool for NYC residents in collaboration with IDEO
- **Keywords / tags:** engineering management, senior software engineer, consulting, team lead, delivery, Python, ETL, Kubernetes, Docker, React, product discovery, logistics, last-mile delivery, QA modernization, mentoring, cross-functional
- **Source(s):** Resume 2026-04-06; user clarification 2026-04-06
- **Ambiguities / follow-up needed:**
  - What delivery/project management responsibilities should be called out explicitly across these engagements?
  - Any concrete outcomes, launch results, or scale metrics for Facebook, Pager, Jetblack, JOOR, or FloodHelp?

### General Assembly
- **Canonical title:** Software Instructor
- **Dates:** 2013-09 to 2014-02
- **Summary:** Taught full-stack web development and mentored students in modern software engineering practices and system-level thinking.
- **Responsibilities / evidence:**
  - Taught full-stack web development
  - Mentored students
  - Supported system-level thinking and engineering practices
- **Keywords / tags:** teaching, mentoring, software engineering, communication, curriculum
- **Source(s):** Resume 2026-04-06; user clarification 2026-04-06
- **Ambiguities / follow-up needed:**
  - What curriculum/tools/topics should be named?

### NY Code & Design Academy
- **Canonical title:** Software Instructor
- **Dates:** 2016-01 to 2016-05
- **Summary:** Taught software development in a separate instructional role from General Assembly.
- **Responsibilities / evidence:**
  - Taught software development
  - Mentored students
  - Supported practical software engineering learning
- **Keywords / tags:** teaching, mentoring, software engineering, communication, curriculum
- **Source(s):** Resume 2026-04-06; user clarification 2026-04-06
- **Ambiguities / follow-up needed:**
  - What curriculum/tools/topics should be named?

## Climate / Mission-Aligned Experience

### Climatebase Fellowship
- **Canonical title:** Climatebase Fellow — Innovator Distinction
- **Dates:** 2025
- **Summary:** Climate-positive fellowship focused on software, data, and decarbonization.
- **Evidence:**
  - Collaborated with climate founders and technologists
  - Built fluency in carbon accounting concepts, decarbonization levers, and climate-driven product strategy
- **Keywords / tags:** climate, decarbonization, carbon accounting, product strategy, fellowship
- **Source(s):** Resume 2026-04-06

### Brine & Ember
- **Canonical title:** Founding developer / capstone project
- **Dates:** 2025
- **Summary:** Circular food venture exploring seaweed-based carbon sequestration and biochar production.
- **Evidence:**
  - Evaluated practical decarbonization approaches for a small business
  - Produced process diagrams, techno-economic assessment, and TAM/SAM/SOM analysis
- **Keywords / tags:** climate, food systems, sequestration, biochar, documentation, analysis
- **Source(s):** Resume 2026-04-06
- **Ambiguities / follow-up needed:**
  - Was this purely a fellowship capstone or an ongoing venture?

## Open Ambiguities Queue

1. Def Method delivery/project management responsibilities that should be called out explicitly
2. Def Method concrete outcomes, launch results, or scale metrics for Facebook, Pager, Jetblack, JOOR, or FloodHelp
3. Frequency Machine / Cerca specific user, launch, or business outcomes
4. Book Bites measurable outcomes for each client engagement
5. General Assembly curriculum/tools/topics
6. NY Code & Design Academy curriculum/tools/topics
7. Any additional roles omitted from this resume version
