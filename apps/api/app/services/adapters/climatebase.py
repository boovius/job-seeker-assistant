...
class ClimatebaseAdapter(SourceAdapter):
    ...
    def fetch_job_detail(self, listing: RawListing, registry: SourceRegistryEntry) -> RawJobDetail:
        ...
        if not job:
            ...
            job = _extract_job_from_detail_html(html)
        structured = {
            "job": job,
...
def _extract_job_from_detail_html(html: str) -> dict[str, Any]:
    title_match = _TITLE_RE.search(html or "")
    description_match = _META_DESCRIPTION_RE.search(html or "")
    ...
    if title:
        job["title"] = title
    if description:
        job["employer_short_description"] = description
    return job
